...
class Cardiologie extends CI_Controller {
	...
	{
		$this->load->view('app/cardiologie/page-liste-avis');
	}	
	...
	public function consultation_avis($id, $avs)
	{
		...
		$this->load->view('app/cardiologie/page-consultation',array("acm_id"=>$id,"avs"=>$avs));
	}
	
	public function mes_patients()
	{
		$this->load->view('app/cardiologie/page-mes-patients');
	}
	
	public function hostirique_de_mes_patients()
	{
		$this->load->view('app/cardiologie/page-dossiers-patients');
	}
	
	public function hostorique_actes()
	{
		$this->load->view('app/cardiologie/page-hostorique-patients');
	}
	...
	public function faire($id)
	{
		...
		$this->md_patient->maj_actes_caisse($donneesAcm,$id);
		$this->load->view('app/cardiologie/page-consultation',array("acm_id"=>$id));
	}
	
	public function voir($id)
	{
		$this->load->view('app/cardiologie/page-rapport-consultation',array("acm_id"=>$id));
	}
	...
	public function ajoutCompteRenduCardiologie(){
		
		date_default_timezone_set('Africa/Brazzaville');
		$data = $this->input->post();
		$config["upload_path"] =  './assets/images/cardiologie/';
		...
		if($this->upload->do_upload("image")){
			...
			$photo="assets/images/cardiologie/".$image['file_name'];
		}
		else{
			$photo=NULL;
		}
		...
		if(isset($_FILES["join"]["name"]) AND $_FILES["join"]["name"]!=""){
			$config2["upload_path"] =  './assets/images/cardiologie/prescription/';
			...
			if($this->upload->do_upload("join")){
				...
				$prescri="assets/images/cardiologie/prescription/".$image2['file_name'];
			}
			else{
				$prescri=NULL;
			}
		...
		$donnees = array(
			"aca_sInterpretation"=>trim($data["interpretation"]),
			"aca_sConclusion"=>trim($data["conclusion"]),
			"aca_dDate"=>date("Y-m-d H:i:s"),
			"aca_iPer"=>$data["idPer"],
			"aca_sTitre"=>$data["titre"],
			"aca_sNom"=>$data["nom"],
			"aca_sPrenom"=>$data["prenom"],
			"aca_sImage"=>$photo,
			"aca_sPrescription"=>$prescri,
			"aca_iSta"=>2
		);
		
		$update = $this->md_patient->maj_acte_medical_cardiologie($donnees,$data["id"]);
		echo "ok";
	}
	...
	public function reponse_avis(){
		...
		$data = $this->input->post();
		if(!isset($data)){
			return redirect("cardiologie/consultation_avis");
		}
		else{
			$donnees = array(
				"avs_iSta"=>2,
			...
			return redirect("cardiologie/demande_avis");
		}
	}
